...
def search_web(query: str) -> str:
    ...
    return "Result for the '{query}' : ... fount 10 matches..."


@tool 
def weather_check(city: str) -> str: 
    """Get the current weather of a city"""
    return "Weather of a {city}: Sunny 25 C"
...
class MultiInputStream(BaseModel):
    a: int = Field(description="first number"),
    b: int = Field(description="second number")

class CustomTool(BaseTool):
    name: str = "calculate product"
...
result = agent.invoke({
    "messages":[{"role":"user", "content":"What's the weather in paris and what 5 + 3"}]
})
print(result)