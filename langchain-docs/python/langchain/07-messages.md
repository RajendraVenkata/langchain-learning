...
<Tip>
  Multi-turn [agents](/oss/python/langchain/agents) accumulate long message histories. [LangSmith](/langsmith/home) records each turn, tool result, and model response so you can inspect the full conversation. Follow the [tracing quickstart](/langsmith/trace-with-langchain) to enable tracing.
